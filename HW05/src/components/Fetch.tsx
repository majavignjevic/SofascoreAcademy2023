...
export default function Fetch() {
  ...
  useEffect(() => {
    const fetchFirstPokemon = async () => {
      const p: PokemonResponse = await (await fetch(getPokemonUrl(idNum))).json()
      setPokemon([p])
    }
    fetchFirstPokemon()
  }, [])
  ...
  const fetchPokemon = async (id: number) => {
    const p: PokemonResponse = await (await fetch(getPokemonUrl(id))).json()
    setPokemon(prevState => [...prevState, p])
  }

  const clickEvent = () =>{
    setIdNum(idNum+1)
    fetchPokemon(idNum+1)
    console.log("Id: "+ idNum +"Next"+ idNum+1)
  }
  ...
  return (
    <div>
      <button onClick={clickEvent}>FETCH</button>
      {
